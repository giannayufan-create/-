...
export default function AuthModal() {
  ...
  const handleEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(msgs[e.code] || e.message || '登入失敗');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 bg-stone-900/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md relative overflow-hidden">
        ...
        <button onClick={() => setAuthModalOpen(false)} className="absolute top-4 right-4 text-stone-400 hover:text-stone-600 z-10">
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="p-8">
          <div className="text-center mb-6">
            ...
            <h2 className="text-2xl font-black text-stone-900">歡迎光臨</h2>
            <p className="text-sm text-stone-500 mt-1">登入後即可訂購美味餐點</p>
          </div>

          <div className="flex bg-stone-100 p-1 rounded-xl mb-5">
            {(['login', 'register'] as const).map((m) => (
              <button key={m} onClick={() => { setMode(m); setError(''); }}
                className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${mode === m ? 'bg-white text-amber-700 shadow-sm' : 'text-stone-500'}`}>
                {m === 'login' ? '登入' : '註冊'}
              </button>
            ))}
          </div>

          {error && <div className="bg-red-50 text-red-700 text-sm p-3 rounded-xl mb-4 border border-red-100">{error}</div>}
          ...
          <form onSubmit={handleEmail} className="space-y-3 mb-5">
            ...
            <div className="relative">
              <input type={showPw ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="密碼"
                className="w-full bg-stone-50 border border-stone-200 rounded-xl p-3 pr-10 text-sm focus:ring-2 focus:ring-amber-400 focus:outline-none" />
            ...
            {mode === 'login' && (
              <button type="button" onClick={() => email && resetPassword(email).then(() => setError('重設密碼信件已寄出')).catch(() => setError('重設密碼失敗'))}
                className="text-xs text-amber-600 font-medium">忘記密碼？</button>
            )}
            <button type="submit" disabled={loading}
              className="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold py-3 rounded-xl transition-all disabled:opacity-50 flex items-center justify-center gap-2">
              {loading ? <><Loader2 className="w-4 h-4 animate-spin" />登入中...</> : mode === 'login' ? '登入' : '註冊帳號'}
            </button>
          </form>

          <div className="relative flex items-center py-2 mb-4">
            <div className="flex-grow border-t border-stone-200" />
            <span className="mx-3 text-stone-400 text-xs">快速登入</span>
            <div className="flex-grow border-t border-stone-200" />
          </div>

          <div className="space-y-2">
            <button disabled={loading} onClick={() => handleProvider('Google', googleSignIn)}
              className="w-full flex items-center justify-center gap-2 border border-stone-200 hover:bg-stone-50 py-2.5 rounded-xl text-sm font-medium disabled:opacity-50">
            ...
            <button disabled={loading} onClick={() => handleProvider('Facebook', facebookSignIn)}
            ...
            <button disabled={loading} onClick={() => handleProvider('Yahoo', yahooSignIn)}
              className="w-full flex items-center justify-center gap-2 bg-[#6001D2] hover:bg-[#4B01A8] text-white py-2.5 rounded-xl text-sm font-medium disabled:opacity-50">
              Yahoo 登入
            </button>
          </div>
        </div>
